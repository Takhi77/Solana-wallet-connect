 ...
 */
export async function loadProgram(customaccount:any): Promise<void> {
  ...
  // var decodedString = Base58.decode(greetedAccount);
  ...
  // greetedAccount =
  // greetedAccount = SimpleBase.decode(greetedAccount, 58)

  // console.log(greetedAccount);
  // greetedPubkey = new PublicKey(greetedAccount);


  // greetedPubkey = greetedAccount;
 ...
 * Say hello
 */
export async function sayHello(message:any): Promise<void> {
  console.log('Send message to', greetedPubkey.toBase58());
  const instruction = new TransactionInstruction({
    keys: [{pubkey: greetedPubkey, isSigner: false, isWritable: true}],
    programId,
    data: Buffer.from(message), // All instructions are hellos
  });
  await sendAndConfirmTransaction(
    connection,
